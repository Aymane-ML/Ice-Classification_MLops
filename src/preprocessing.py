...
def load_data(
    ...
    """Charge les fichiers de données d'entrée (features)
    et de sortie (targets) depuis des chemins CSV.

    Args:
        features_path (str): Chemin du fichier CSV contenant les features.
        Defaults to "data/raw/data_Features.csv".
        targets_path (str): Chemin du fichier CSV contenant la target.
        Defaults to "data/raw/data_Targets.csv".

    Returns:
        Tuple[pd.DataFrame, pd.DataFrame]: Deux DataFrames :
        df_features (X) et df_targets (y).
    """
...
def preprocess_features(
    ...
    """Transforme les colonnes temporelles en variables numériques.

    Args:
        df_features (pd.DataFrame):  DataFrame contenant les features,
        incluant une colonne 'time'.

    Returns:
        pd.DataFrame: DataFrame nettoyé avec les colonnes :
        'year', 'month', 'day' à la place de 'time'.
    """
    df = df_features.copy()
